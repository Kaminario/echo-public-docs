...
| File | Purpose |
|------|---------|
| `setup.ps1` | Interactive multi-host installation orchestrator |
| `setup-one-host.ps1` | Standalone single-host installation script |
| `host-setup-config.json` | Multi-host configuration file |
...
.\setup-one-host.ps1 -FlexIP "10.0.0.1" -FlexToken "your-token" -DBConnectionString "server=localhost;..." -SilkAgentURL "https://..." -SilkVSSURL "https://..." -SDPId "d9b601" -SDPUsername "admin" -SDPPassword "password"

# Validation mode (dry run)
.\setup-one-host.ps1 -FlexIP "10.0.0.1" -FlexToken "your-token" -DBConnectionString "server=localhost;..." -SilkAgentURL "https://..." -SilkVSSURL "https://..." -SDPId "d9b601" -SDPUsername "admin" -SDPPassword "password" -DryRun
```

### Multi-Host Setup
```powershell
# Install across multiple hosts using config file
.\setup.ps1 -ConfigPath "host-setup-config.json"

# Use custom configuration with different MaxConcurrency
.\setup.ps1 -ConfigPath "custom-config.json" -MaxConcurrency 5

# Validation mode (dry run)
.\setup.ps1 -ConfigPath "host-setup-config.json" -DryRun

# Combined options
.\setup.ps1 -ConfigPath "config.json" -MaxConcurrency 3 -DryRun
```

## Configuration

Edit `host-setup-config.json` for multi-host deployments:
...
Both `setup.ps1` and `setup-one-host.ps1` scripts support standard PowerShell features:

- **Get-Help**: Use `Get-Help .\setup.ps1` or `Get-Help .\setup-one-host.ps1` to view detailed parameter information and examples
...
Get-Help .\setup-one-host.ps1 -Full

# Run with debug level output
.\setup.ps1 -ConfigPath "config.json" -Verbose

# Run with debug level output
.\setup-one-host.ps1 -FlexIP "10.0.0.1" -FlexToken "token" -DBConnectionString "..." -SilkAgentURL "..." -SilkVSSURL "..." -SDPId "d9b601" -SDPUsername "admin" -SDPPassword "password" -Debug
```
