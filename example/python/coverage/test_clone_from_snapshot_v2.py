...
"""
Test clone from snapshot endpoints.

Tests:
- POST /api/ocie/v1/db_snapshots/{id}/clone/__validate (validation)
- POST /api/ocie/v1/db_snapshots/{id}/clone (direct endpoint)
...
def run(
    ...
    dest_host_name: str,
    db_name: str,
    snapshot_prefix: str = "test-clone-",
    echo_db_suffix: str = "-clone",
    ...
    Args:
        ...
        snapshot_prefix: Prefix for snapshot name (default: "test-clone-")
        echo_db_suffix: Suffix for cloned echo DB name (default: "-clone")
    ...
    print(f"Testing clone from snapshot endpoints")
    ...
    try:
        ...
        print("\n2. Testing POST /api/ocie/v1/db_snapshots/{id}/clone/__validate")
        validate_response = _make_request(
            "POST",
            f"/api/ocie/v1/db_snapshots/{snapshot_id}/clone/__validate",
            payload=clone_payload
        )
        print("   ✓ Validation passed")

        # Test direct endpoint
        print("\n3. Testing POST /api/ocie/v1/db_snapshots/{id}/clone")
        ...
        if snapshot_id:
            try:
                delete_snapshot_task = _make_request("DELETE", f"/api/ocie/v1/db_snapshots/{snapshot_id}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test clone from snapshot endpoints")
    ...
    parser.add_argument("--snapshot-prefix", default="test-clone-", help="Prefix for snapshot name (default: 'test-clone-')")
    parser.add_argument("--echo-db-suffix", default="-clone", help="Suffix for cloned echo DB name (default: '-clone')")
