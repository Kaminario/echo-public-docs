...
"""
Test echo DB clone endpoints (secondary API for backward compatibility).

Tests:
- POST /api/ocie/v1/clone/__validate (validation)
- POST /api/ocie/v1/clone (secondary endpoint - creates snapshot + echo DB)
- DELETE /api/ocie/v1/clone (delete echo DB)
...
def run(
    ...
    dest_host_name: str,
    db_name: str,
    echo_db_suffix: str = "-test",
    consistency_level: str = "crash",
    timeout: int = 300,
):
    """Test echo DB clone endpoints (direct clone from source).

    Args:
        source_host_name: Source host name/ID
        dest_host_name: Destination host name/ID
        db_name: Database name to clone
        echo_db_suffix: Suffix for cloned echo DB name (default: "-test")
    ...
    print(f"Testing echo DB clone endpoints (direct clone)")
    ...
    try:
        ...
        print("\n1. Testing POST /api/ocie/v1/clone/__validate")
        validate_response = _make_request("POST", "/api/ocie/v1/clone/__validate", payload=payload)
        print("   ✓ Validation passed")

        # Test direct endpoint
        print("\n2. Testing POST /api/ocie/v1/clone")
        task = _make_request("POST", "/api/ocie/v1/clone", payload=payload)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test echo DB clone endpoints (direct clone from source)")
    ...
    parser.add_argument("--echo-db-suffix", default="-test", help="Suffix for cloned echo DB name (default: '-test')")
